...
Kalman Filtering can be performed online, however if we know the values of all the observations of interest ahead of time, we can perform smoothing.
...
Inverse gamma priors on the system and observation variances, $p(V) = \textrm{InverseGamma}(4.0, 9.0)$, meaning the prior mean of the observation variance is $\mathbb{E}(V) = 9 / 3 = 3$ and the variance is $\textrm{Var}(V) = $.

To perform gibbs sampling with inverse gamma priors:

```scala
val iters = gibbsSamples(mod, InverseGamma(1.0, 10.0), InverseGamma(1.0, 10.0), p, data.map(_._1))
```
