...
```{r stochastic-volatility-parameters-svd, eval=FALSE}
...
actual_values = tibble(
  Parameter = c("v", paste0(c("phi", "mu", "sigma_eta"), rep(1:2, each = 3)), paste0("beta", 1:12)),
  actual_value = c(0.1, rep(x = c(0.8, 2.0, 0.2), times = 2), beta)
)
chain1 = read_csv("../examples/data/factor_sv_gibbs_0.csv", col_names = actual_values$Parameter)
chain2 = read_csv("../examples/data/factor_sv_gibbs_1.csv", col_names = actual_values$Parameter)
chains = ggs(mcmc.list(mcmc(chain1[seq(1, nrow(chain1), by = 10),]), mcmc(chain2[seq(1, nrow(chain2), by = 10),])))
```
...
p1 = chains %>%
  ...
  facet_wrap(~Parameter, scales = "free_y", ncol = 2, strip.position = "right")
...
p3 = chains %>%
  ...
  geom_vline(aes(xintercept = actual_value), colour = "#ff0000", linetype = "dashed") +
  facet_wrap(~Parameter, scales = "free", ncol = 2, strip.position = "right") +
  theme(legend.position = "none")
