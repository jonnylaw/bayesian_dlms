...
first_order_and_linear_trend = read_csv("../core/data/first_order_and_linear_trend.csv")
...
data = read_csv("../core/data/correlated_dlm.csv")
...
```{r filter-correlated}
filtered = read_csv("../core/data/correlated_dlm_filtered.csv", 
...
iters = read_csv("../core/data/correlated_dlm_gibbs.csv", col_names = c(paste0("V", 1:4), paste0("W", 1:4)))
...
p1 = params %>%
  ...
  labs(title = "Diagnostic plots for the Observation noise covariance matrix",
       subtitle = "(Top) Traceplots (Middle) Autocorrelation (Bottom) Empirical density")
...
p1 = params %>%
  ...
  geom_hline(aes(yintercept = actual_value), colour = "#ff0000") +
  facet_wrap(~Parameter, scales = "free_y") +
  labs(title = "Diagnostic plots for the System noise covariance matrix",
       subtitle = "(Top) Traceplots (Middle) Autocorrelation (Bottom) Empirical density")
...
p3 = params %>% 
  filter(Parameter %in% paste0("W", c(1,2,4))) %>%
  ggs_density() +
  facet_wrap(~Parameter, ncol = 3, scales = "free_x", strip.position = "right")
