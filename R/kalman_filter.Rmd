...
```tut:silent
val filtered = SvdFilter.filter(mod, data.map(_._1), p)
```

The figure below shows the SVD filter ran on the first order DLM example.

```{r filter-svd, echo=FALSE}
filtered_svd = read_csv("../data/first_order_dlm_svd_filtered.csv")
