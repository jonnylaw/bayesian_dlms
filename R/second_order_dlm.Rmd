...
data = read_csv("../core/data/second_order_dlm.csv")
...
filtered = read_csv("../core/data/second_order_dlm_filtered.csv")
...
gibbs_iters = read_csv("../core/data/second_order_dlm_gibbs.csv")
...
gibbs_iters %>%
  ...
  group_by(Parameter) %>%
  summarise(mean = mean(value), lower = quantile(value, probs = 0.025), 
            upper = quantile(value, probs = 0.975), sd = sd(value)) %>%
  inner_join(actual_values) %>%
  knitr::kable(caption = "Summary Statistics of the parameters of the second order DLM", digits = 2)
```
