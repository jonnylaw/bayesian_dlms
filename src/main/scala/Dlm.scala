...
object Dlm {
  ...
  /**
    ...
    */
  def regression(x: Array[Data]): Model = {

    Model(
      (t: Time) => {
        val m = 1 + x(t).observation.size
        new DenseMatrix(1, m, 1.0 +: x(t).observation.data)
      },
      (t: Time) => DenseMatrix.eye[Double](x.head.observation.size)
    )
  }
  ...
  /**
    ...
    */
  def rotation_matrix(theta: Double): DenseMatrix[Double] = {
  ...
  /**
    ...
    */
  def block_diagonal(a: DenseMatrix[Double], 
  ...
  def seasonal(period: Int, harmonics: Int): Model = {
    ...
    val matrices = (1 to harmonics) map (h => rotation_matrix(freq * h))

    Model(
      (t: Time) => DenseMatrix.tabulate(1, harmonics * 2){ case (i, h) => h % 2 },
      (t: Time) => matrices.reduce(block_diagonal)
    )
  }
  ...
  /**
    ...
    */
  def backwards_smoother(mod: Model, observations: Array[Data], p: Parameters) = ???

  /**
    * Minimise the negative log-likelihood using an optimisation routing in Breeze
    */
  def mle(mod: Model, observations: Array[Double])(init_p: Parameters) = ???
