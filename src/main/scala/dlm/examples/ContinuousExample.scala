...
//   val sims = Dlm.simulate(times, mod, p)
...
//   val states = Monad[Rand].replicateA(100, GibbsSampling.sampleState(mod, observations, p)).
//     draw.
//     flatten.
//     groupBy(_._1).
//     map { case (time, state) => 
...
//   val iters = GibbsSampling.sample(mod, InverseGamma(5.0, 4.0),
