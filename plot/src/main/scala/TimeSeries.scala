...
object TimeSeries {
  /**
    ...
    */
  def plotWithState(xs: Vector[(Double, DenseVector[Option[Double]], DenseVector[Double])]): Unit = ???

  /**
    * Convert a vector of Data into a vector of vector of points which can each be plotted
    */
  def listOfObservations(
    xs: Vector[(Double, DenseVector[Option[Double]])]): Vector[Vector[Point]] = {

    val times: Vector[Double] = xs.map(_._1)
    val states: Vector[Vector[Option[Double]]] = xs.map(_._2.data.toVector).transpose
  ...
  /**
    ...
    */
  def plotObservations(xs: Vector[(Double, DenseVector[Option[Double]])]): Plot = {
    val plots = for {
