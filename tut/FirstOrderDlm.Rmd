...
$$\begin{aligned}
Y_t &= x_t + v_t, &v_t &\sim \mathcal{N}(0, V), \\
X_t &= x_{t-1} + w_t, &w_t &\sim \mathcal{N}(0, W), \\
...
scala> val data = Dlm.simulateRegular(0, mod, p).
     ...
     |   take(1000).
     |   toArray
data: Array[(dlm.model.Data, breeze.linalg.DenseVector[Double])] = Array((Data(1.0,Some(DenseVector(1.89231026460599))),DenseVector(-0.4796746561779635)), (Data(2.0,Some(DenseVector(-2.120707748039276))),DenseVector(-2.8473531017989915)), (Data(3.0,Some(DenseVector(-3.7028676769165783))),DenseVector(-1.5697876601639127)), (Data(4.0,Some(DenseVector(1.4223228462583482))),DenseVector(-1.0257628398193774)), (Data(5.0,Some(DenseVector(-4.819173896310765))),DenseVector(-2.7717231963533937)), (Data(6.0,Some(DenseVector(-0.8950621328577546))),DenseVector(-2.762610316004954)), (Data(7.0,Some(DenseVector(-5.625485073909319))),DenseVector(-3.7858858064496803)), (Data(8.0,Some(DenseVector(-4.129330259712452))),DenseVector(-2.612259314858031)), (Data(9.0,Some(DenseVector(-3.1816864897832264))),Dens...
```
...
scala> Dlm.Model(
     ...
     | )
res0: dlm.model.Dlm.Model = Model(<function1>,<function1>)
```
...
filtered: Array[dlm.model.KalmanFilter.State] = Array(State(0.0,DenseVector(0.0),1.0  ,DenseVector(0.0),1.0  ,None,None,0.0), State(1.0,DenseVector(0.756924105842396),1.2000000000000002  ,DenseVector(0.0),2.0  ,Some(DenseVector(0.0)),Some(5.0  ),-2.081741303175042), State(2.0,DenseVector(-0.4605355246460038),1.2692307692307692  ,DenseVector(0.756924105842396),2.2  ,Some(DenseVector(0.756924105842396)),Some(5.2  ),-4.621236561334412), State(3.0,DenseVector(-1.85686834934647),1.2919708029197077  ,DenseVector(-0.4605355246460038),2.269230769230769  ,Some(DenseVector(-0.4605355246460038)),Some(5.269230769230769  ),-7.368674450584144), State(4.0,DenseVector(-0.4366393349741764),1.2993103448275862  ,DenseVector(-1.85686834934647),2.2919708029197077  ,Some(DenseVector(-1.85686834934647)),Some(...
```
...
scala> val iters = GibbsSampling.sample(
     ...
     |     p, 
     |     data.map(_._1))
iters: breeze.stats.distributions.Process[dlm.model.GibbsSampling.State] = breeze.stats.distributions.MarkovChain$$anon$1@26b09e87
```
