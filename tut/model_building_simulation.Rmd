...
```scala
scala> val linear = Dlm.polynomial(1)
linear: dlm.model.Dlm.Model = Model(<function1>,<function1>)

scala> val seasonal = Dlm.seasonal(period = 24, harmonics = 3)
seasonal: dlm.model.Dlm.Model = Model(<function1>,<function1>)
```

```scala
val x: Array[Data] = ??? // timeseries to regress against
...
scala> val seasonalTrend = linear |+| seasonal
seasonalTrend: dlm.model.Dlm.Model = Model(<function1>,<function1>)
```

The models can also be combined using a cartesian product in order to model multivariate time series:

```scala
scala> // model for a bivariate time series
     | val multivariateModel = linear |*| seasonalTrend
multivariateModel: dlm.model.Dlm.Model = Model(<function1>,<function1>)

scala> // model for 30 seemingly unrelated time series
     | val sutse = List.fill(30)(linear).reduce(_ |*| _)
sutse: dlm.model.Dlm.Model = Model(<function1>,<function1>)
```
...
```scala
scala> val poisson = Dglm.poisson(linear)
poisson: dlm.model.Dglm.Model = Model(<function2>,<function1>,<function1>,<function1>)
```
...
scala> val p = Dlm.Parameters(
     ...
     |   c0 = DenseMatrix(10.0)
     | )
p: dlm.model.Dlm.Parameters = Parameters(1.0  ,3.0  ,DenseVector(0.0),10.0  )
```

Then we can simulate from the model:

```scala
scala> val data = Dlm.simulateRegular(0, linear, p).
     |   steps.
     |   take(1000)
data: Iterator[(dlm.model.Data, breeze.linalg.DenseVector[Double])] = non-empty iterator
```
...
```scala
scala> Dglm.simulate(poisson, p)
res2: breeze.stats.distributions.Process[(dlm.model.Data, breeze.linalg.DenseVector[Double])] = breeze.stats.distributions.MarkovChain$$anon$1@55fdac85
```
