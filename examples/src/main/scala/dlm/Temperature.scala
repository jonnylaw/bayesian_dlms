...
trait TemperatureModel {
  ...
  val mvDlmShareState = seasonalDlm.copy(f = (t: Double) =>
    List.fill(8)(seasonalDlm.f(t)).reduce((a, b) => DenseMatrix.horzcat(a, b)))
  ...
  val ys = data
    ...
    .map {
      case (t, temps) =>
        Data(
          t.toEpochSecond(ZoneOffset.UTC) / (60.0 * 60.0 * 24), // time in days
...
object TemperatureDlm extends App with TemperatureModel {
  ...
  val prior = for {
    ...
    DlmParameters(v = diag(DenseVector(v.toArray)),
                  w = diag(DenseVector(w.toArray)),
                  m0 = DenseVector(m0),
                  c0 = diag(DenseVector(c0)))

  val iters =
    GibbsSampling.sample(mvDlmShareState, priorV, priorW, prior.draw, ys)
  ...
  Streaming
    .writeParallelChain(iters,
                        2,
                        100000,
                        "examples/data/temperature_dlm_share_state",
                        formatParameters)
...
object ForecastTemperatureDlm extends App with TemperatureModel {
  ...
  } yield io).onComplete(_ => system.terminate())
}
...
object StateTemperatureDlm extends App with TemperatureModel {
  ...
  val res = for {
    p <- ps
    state = Smoothing
      .ffbsDlm(mvDlmShareState, ys, p)
      .sample(1000)
      .map(_.map(x => x.copy(sample = mvDlmShareState.f(x.time).t * x.sample)))
    times: Vector[LocalDateTime] = data.map(_.date)
    meanState = state.transpose
      .zip(times)
      .map {
        case (x, t) =>
          (t, x.map(_.sample).reduce(_ + _).map(_ / 1000).data.toList)
      }
      .toList
  ...
  (for {
    ...
    io <- Streaming
      .writeParallelChain(iters,
                          2,
                          10000,
                          "examples/data/temperature_student_dlm",
                          formatParameters)
      .runWith(Sink.head)
  } yield io)
    .onComplete { s =>
    ...
    .mapAsync(1) { params =>
      ...
      for {
        ...
        test <- encodedData
          ...
          .runWith(Sink.seq)
          .map(_.toVector.sortBy(_.time))
        p = params.copy(m0 = DenseVector.zeros[Double](7),
        ...
        forecast = Dglm
          .forecastParticles(model, lastState, p, test)
  ...
  Streaming
    ...
    .mapAsync(1) { params =>
      ...
      for {
        ...
        // sample the state
        state = StudentT
          .interpolate(test, model, 20, p.copy(m0 = ls._1, c0 = ls._2))
          .steps
          .take(1000)
        ...
        observation = state
          .map(_.state)
          .map(s =>
            s.map(x => Dglm.observation(model, p, x.sample, x.time).draw))

        io <- Future.successful(
          out.writeCsv(observation.map(_.map(x => x(0))), headers))
      } yield io
    }
    .runWith(Sink.onComplete { s =>
      println(s)
