...
trait RoundedUoData {
  ...
  case class EnvSensor(datetime: LocalDateTime,
                       sensorId: String,
                       co: Option[Double],
  ...
  val data = Streaming
    .readCsv("examples/data/summarised_sensor_data.csv")
    .map(parseEnvSensor)
    .collect { case Some(a) => a }

  val encodedData = Streaming
    .readCsv("examples/data/encoded_sensor_data.csv")
    .map(parseEnvSensor)
    .collect { case Some(a) => a }
...
object FitUoDlms extends App with EmoteData {
  ...
  def writeSensor(id: String): Future[Done] = {
    ...
    val iters =
      GibbsSampling.sample(dlmComp, priorV, priorW, dlmP.draw, d.toVector)
    Streaming
      .writeParallelChain(iters,
                          2,
                          10000,
                          s"examples/data/uo_dlm_seasonal_daily_log_no_${id}",
...
object ForecastUoDlmLog extends App with RoundedUoData {
  ...
  val file =
    s"examples/data/uo_dlm_seasonal_daily_log_no_new_new_emote_2603_0.csv"
  ...
  val out = new java.io.File(
    s"examples/data/forecast_urbanobservatory_logno_new_new_emote_2603.csv")
  ...
  (for {
    ...
    data <- sensor2603
    times = data.map(_._1)
    filtered = KalmanFilter.filterDlm(dlmComp, data.map(_._2).toVector, p)
    summary = filtered.flatMap(kf =>
      (for {
        f <- kf.ft
        q <- kf.qt
      } yield Dlm.summariseForecast(0.75)(f, q)).map(f => f.flatten.toList))
    toWrite: Seq[(String, LocalDateTime, List[Double])] = times
      .zip(summary)
      .map(d => ("new_new_emote_2603", d._1, d._2))
...
object SmoothUo extends App with RoundedUoData {
  ...
  data
    ...
    .map { d =>
      ...
      val file =
        s"examples/data/uo_dlm_seasonal_daily_log_no_new_new_emote_2604_0.csv"
...
object ForecastUoDlm extends App with RoundedUoData {
  ...
  encodedData
    ...
    .mergeSubstreams
    .mapAsyncUnordered(1) {
      case (id: String, d: Vector[(LocalDateTime, Data)]) =>
        val data = d.map(_._2)
        ...
        val out =
          new java.io.File(s"examples/data/forecast_urbanobservatory_${id}.csv")
        val headers = rfc.withHeader(false)
