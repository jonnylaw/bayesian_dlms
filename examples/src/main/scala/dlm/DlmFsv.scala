...
trait DlmFsvModel {
  ...
  val params = FsvParameters(v = DenseMatrix.eye[Double](6) * 0.1,
                             beta,
                             Vector.fill(k)(SvParameters(0.8, 2.0, 0.2)))

  val foP = DlmParameters(v = DenseMatrix(2.0),
                          w = DenseMatrix(3.0),
                          m0 = DenseVector(0.0),
                          c0 = DenseMatrix(1.0))
  val dlmP = List.fill(6)(foP).reduce(Dlm.outerSumParameters)
...
trait SimulatedDlmFsv {
  ...
  val data = reader.collect {
    case Right(a) =>
      Data(a.head, DenseVector(a.slice(1, 7).map(_.some).toArray))
  }.toVector
}
...
object InterpolateDlmFsv extends App with DlmFsvModel {
  ...
  val data = reader
    .collect {
      case Right(a) =>
        Data(a.head, DenseVector(a.drop(1).take(6).toArray.map(_.some)))
    }
    .toVector
    .map(d =>
      if (d.time > 700 && d.time < 800) {
...
trait DlmFsvSystemModel {
  ...
  val dlmMod = Dlm.polynomial(1) |+| Dlm.seasonal(24, 3) |+| Dlm.seasonal(
    24 * 7,
    3)
  ...
  val dlmP = DlmParameters(v = DenseMatrix(0.5),
...
object SimulateDlmFsvSystem extends App with DlmFsvSystemModel {
  val sims = DlmFsvSystem.simulateRegular(dlmMod, params, 1).steps.take(5000)
...
object FitDlmFsvSystem extends App with DlmFsvSystemModel {
  ...
  Streaming
    .writeParallelChain(iters,
                        2,
                        100000,
                        "examples/data/dlm_fsv_system_params",
                        formatParameters)
    .runWith(Sink.onComplete(_ => system.terminate()))
}
