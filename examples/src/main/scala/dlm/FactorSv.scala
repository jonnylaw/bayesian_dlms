...
object SimulateFsv extends App with FsvModel {
  ...
  def formatData(d: (Data, Vector[Double], Vector[Double])): List[Double] =
    d match {
      case (Data(t, y), f, a) =>
...
object FitFsv extends App with FsvModel {
  ...
  val data = reader.collect {
    case Right(a) =>
      Data(a.head, DenseVector(a.slice(1, 7).toArray.map(_.some)))
  }.toVector
...
object SampleStateFsv extends App with FsvModel {
  ...
  val data = reader
    .collect {
      case Right(a) =>
        Data(a.head, DenseVector(a.drop(1).take(p).toArray.map(_.some)))
    }
    .toVector
    .take(100)
  ...
  val summary = iters.transpose.map { x =>
    ...
    (t + 1,
     mean(sample),
     Streaming.quantile(sample, 0.995),
     Streaming.quantile(sample, 0.005))
  }
