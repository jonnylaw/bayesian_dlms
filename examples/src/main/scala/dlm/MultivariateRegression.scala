...
trait MultivariateRegression {
  ...
  def model(x: Vector[Array[Double]]) =
    x.map(xi => Dlm.regression(xi.map(DenseVector(_)))).reduce(_ |*| _)
...
object UnivariateRegression extends App {
  val n = 300
  val xs =
    Array.iterate(0.0, n)(x => x + Uniform(-1, 1).draw).map(x => DenseVector(x))
  val params = DlmParameters(
  ...
  val sims =
    Dlm.simulateRegular(Dlm.regression(xs), params, 1.0).steps.take(n).toVector
  ...
  def formatData(d: ((Data, DenseVector[Double]), DenseVector[Double])) =
    d match {
      case ((Data(t, y), s), x) =>
        t :: KalmanFilter
          .flattenObs(y)
          .data
          .toList ::: s.data.toList ::: x.data.toList
    }
...
object SimulateMultivariateRegression extends App with MultivariateRegression {
  ...
  val sims =
    Dlm.simulateRegular(model(xs), params, 1.0).steps.take(300).toVector
...
object FitMultivariateRegression extends App with MultivariateRegression {
  ...
  val data = reader.collect {
    case Right(a) =>
      (Data(a.head, DenseVector(a.drop(1).take(4).toArray).map(_.some)),
       a.drop(13).toArray)
  }.toVector
  ...
  val iters = GibbsSampling.sample(model(xs.map(_.toArray)),
                                   priorV,
                                   priorW,
                                   params,
                                   data.map(_._1))
  ...
  Streaming
    .writeParallelChain(iters,
                        2,
                        10000,
                        "examples/data/multivariate_regression_gibbs",
                        formatParameters)
