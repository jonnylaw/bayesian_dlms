...
object SimulateSv extends App {
  ...
  val sims = StochasticVolatility.simulate(p).steps.take(10000).toVector.tail
...
object FitSv extends App {
  ...
  val iters =
    StochasticVolatility.sampleUni(priorPhi, priorMu, priorSigma, data)
...
object FitSvMixBeta extends App {
  ...
  val iters =
    StochasticVolatility.sampleBeta(priorPhi, priorMu, priorSigma, data)
...
object SvSampleStateMixture extends App {
  ...
  Streaming
    ...
    .map { params =>
      ...
      val iters = MarkovChain(initState.draw)(
        StochasticVolatility.sampleStateAr(data, params, _)).steps
        .take(1000)
        .toVector
      ...
      val summary = iters.transpose.map { x =>
        ...
        (x.head.time,
         mean(xt),
         Streaming.quantile(xt, 0.95),
         Streaming.quantile(xt, 0.05))
      }
...
object SvSampleStateKnots extends App {
  ...
  Streaming
    ...
    .via(Streaming.meanSvParameters)
    .map { p =>
      val sampleStep = (st: Array[FilterAr.SampleState]) =>
        for {
      ...
      val iters = MarkovChain(initState)(sampleStep).steps
        .take(1000)
...
object SimulateOu extends App {
  ...
  val sims = StochasticVolatility.simOu(p, times).take(10000).toVector
...
object FitSvOu extends App {
  ...
  val iters =
    StochasticVolatilityKnots.sampleOu(priorPhi, priorMu, priorSigma, data, p)
