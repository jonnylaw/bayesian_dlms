...
trait AqmeshModel {
  ...
  val fsv = for {
    ...
    FsvParameters(DenseMatrix.eye[Double](21) * sigmaX,
  ...
  val initP = for {
    ...
    dlm = DlmParameters(diag(DenseVector.fill(3)(v)),
                        DenseMatrix.eye[Double](21),
                        m0,
                        c0),
...
trait AqMeshData {
  ...
  case class AqmeshSensor(datetime: LocalDateTime,
...
object FitAqMeshDlm extends App with AqMeshData {
  ...
  val dlmComp = (List.fill(5)(seasonal24) ++ List.fill(2)(Dlm.polynomial(1)))
    .reduce(_ |*| _)
  ...
  val initP = for {
    ...
    DlmParameters(diag(DenseVector.fill(7)(v)),
                  diag(DenseVector.fill[Double](37)(w)),
                  m0,
                  c0)
  ...
  val training = reader
    ...
    .filter(
      _.datetime.compareTo(LocalDateTime.of(2018, Month.JANUARY, 1, 0, 0)) > 0)
    .filter(
      _.datetime.compareTo(LocalDateTime.of(2018, Month.MARCH, 1, 0, 0)) < 0)
    .toVector
    .zipWithIndex
  ...
  val iters =
    GibbsSampling.sample(dlmComp, priorV, priorW, initialParams, training)
  ...
  Streaming
    .writeParallelChain(iters,
                        2,
                        100000,
                        "examples/data/aqmesh_dlm_params",
                        formatParameters)
...
object FitAqMeshFull extends App with AqMeshData {
  ...
  val dlmComp = List.fill(7)(seasonalDlm).reduce(_ |*| _)
  ...
  val fsv = for {
    ...
    FsvParameters(DenseMatrix.eye[Double](7) * sigmaX,
  ...
  val training = reader
    ...
    .filter(
      _.datetime.compareTo(LocalDateTime.of(2018, Month.JANUARY, 1, 0, 0)) > 0)
    .filter(
      _.datetime.compareTo(LocalDateTime.of(2018, Month.FEBRUARY, 1, 0, 0)) < 0)
    .toVector
    .zipWithIndex
    .filter { case (_, i) => i % 4 == 0 }
    . // thinned
  ...
  Streaming
    .writeParallelChain(iters,
                        2,
                        100000,
                        "examples/data/aqmesh_gibbs_full",
                        formatParameters)
    .runWith(Sink.onComplete { s =>
      println(s)
      system.terminate()
...
object OneStepForecastAqmesh extends App with AqmeshModel with AqMeshData {
  ...
  val data = reader.collect { case Right(a) => a }.toVector
  ...
  val test = data
    .filter(
      _.datetime.compareTo(LocalDateTime.of(2018, Month.FEBRUARY, 1, 0, 0)) > 0)
    .filter(
      _.datetime.compareTo(LocalDateTime.of(2018, Month.FEBRUARY, 7, 0, 0)) < 0)
    .zipWithIndex
    .filter { case (_, i) => i % 4 == 0 }
    . // thinned
    map(_._1)
  ...
  Streaming
    ...
    .map { params =>
      ...
      val summary = forecast.flatMap(x =>
        (for {
          f <- x.ft
          q <- x.qt
